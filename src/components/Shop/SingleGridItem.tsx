"use client"; // اعلام می‌کنه که این کامپوننت در سمت کلاینت رندر میشه (مخصوص Next.js 13 به بالا)

import React from "react";
import { Product } from "@/types/product"; // ایمپورت تایپ محصول برای TypeScript
import { useModalContext } from "@/app/context/QuickViewModalContext"; // کانتکست مسئول باز/بسته شدن مودال "مشاهده سریع"
import { updateQuickView } from "@/redux/features/quickView-slice"; // اکشن مربوط به به‌روزرسانی اطلاعات محصول در حالت مشاهده سریع
...
import { useDispatch } from "react-redux"; // هوک برای ارسال اکشن به Redux
import { AppDispatch } from "@/redux/store"; // تایپ مخصوص dispatch در پروژه
import Link from "next/link"; // لینک داخلی Next.js
import Image from "next/image"; // بهینه‌سازی و نمایش عکس‌ها در Next.js

// نمایش یک محصول به شکل کارت با امکانات: مشاهده سریع، افزودن به سبد خرید و لیست علاقه‌مندی‌ها
const SingleGridItem = ({ item }: { item: Product }) => {
  
  console.log( "SingleGridItem page" );
  console.log(item);
  

  const { openModal } = useModalContext(); // گرفتن تابع باز کردن مودال از کانتکست

  const dispatch = useDispatch<AppDispatch>(); // تابع dispatch برای ارسال اکشن‌ها به Redux

  // تابعی برای آپدیت‌کردن state مربوط به Quick View
  ...
  const handleAddToCart = () => {
    dispatch(
      addItemToCart({
        ...item,
        quantity: 1, // پیش‌فرض اضافه کردن یک عدد
      })
    );
  };
  ...
  return (
    <div className="group">
      ...
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        ...
        {/* دکمه‌ها که هنگام هاور روی کارت نمایش داده می‌شوند */}
