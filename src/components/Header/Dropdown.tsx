...
// کامپوننت Dropdown: برای منوی کشویی هر آیتم ناوبار
const Dropdown = ({ menuItem, stickyMenu }) => {
  // مدیریت باز و بسته شدن منوی کشویی
  const [dropdownToggler, setDropdownToggler] = useState(false);

  // گرفتن مسیر فعلی صفحه برای فعال کردن آیتم‌های منو
  const pathUrl = usePathname();

  return (
    // هر آیتم منو (LI) که کلیک روی آن باعث باز شدن زیرمنو می‌شود
    <li
      onClick={() => setDropdownToggler(!dropdownToggler)}
      className={`group relative 
        before:w-0 before:h-[3px] before:bg-blue before:absolute 
        before:left-0 before:top-0 before:rounded-b-[3px] before:ease-out 
        before:duration-200 hover:before:w-full ${
    ...
    >
      ...
      >
        {/* پیمایش روی آیتم‌های زیرمنو */}
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-blue hover:bg-gray-1 
                py-[7px] px-4.5 ${
